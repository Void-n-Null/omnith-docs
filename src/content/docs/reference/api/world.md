...
| `Get``1` | Get a reference to an entity's component. Entity must have the component. |
...
| `ParallelEach``2` | Parallel iteration over all entities with two components. Callback receives raw component refs only (no Entity handle). Safe for systems where each entity's update is independent. Splits work across 8 threads within each archetype table. |
