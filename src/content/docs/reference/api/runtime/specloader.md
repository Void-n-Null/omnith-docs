...
Loads .data files (JSONC arrays of specs) and resolves spec references. For each field on a spec type: - If the type inherits from [Spec](/reference/api/specs/spec/) and the JSON value is a string: lookup by ID - If the type inherits from [Spec](/reference/api/specs/spec/) and the JSON value is an object: deserialize, auto-assign ID as "parentId.fieldName", register, and assign - Otherwise: deserialize directly Uses SpecTypeCache for cached reflection (reflected once per type, not per instance).
...
| `DeserializeSpec` | Deserialize a JSON element into a [Spec](/reference/api/specs/spec/). Handles spec-reference fields: string -> deferred (resolved after all specs loaded) object -> inline spec, auto-ID'd and registered immediately |
