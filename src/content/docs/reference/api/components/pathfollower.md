...
Pathfinding state. References a path in the external [PathStore](/reference/api/stores/pathstore/).

## Fields

| Name | Description |
|---|---|
| `PathId` | Index into [PathStore](/reference/api/stores/pathstore/). -1 = no path. |
| `StepIndex` | Current step along the path. |
| `ComputedVersion` | Last [PathTarget](/reference/api/components/pathtarget/).Version we computed for. If stale, recompute. |
